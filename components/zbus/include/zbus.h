...
#include <stdint.h>
#include <string.h>
#define k_timeout_t uint32_t
#define sys_clock_tick_get() (xTaskGetTickCount() / portTICK_PERIOD_MS)
#define k_is_in_isr xPortInIsrContext

#define k_msgq QueueDefinition
#define k_mutex QueueDefinition

// #define k_msgq_put xQueueSend
// #define k_msgq_get xQueueReceive
// #define k_mutex_lock xSemaphoreTake
// #define k_mutex_unlock xSemaphoreGive

// #define __ASSERT(_cond, _fmt, ...) assert(_cond)
#define __ASSERT(_cond, _fmt, ...)
#define LOG_ERR(_fmt, ...) ESP_LOGE("ZBUS", _fmt, ##__VA_ARGS__)
...
#define CONFIG_ZBUS_STRUCTS_ITERABLE_ACCESS
 ...
 *
 * Every channel has a zbus_channel structure associated used to control the
 * channel access and usage.
 */
struct zbus_channel {
  ...
  /** User data available to extend zbus features. The channel must be claimed
   * before using this field.
   */
  void *const user_data;

  /** Message reference. Represents the message's reference that points to the
   * actual shared memory region.
   */
  void *const message;

  /** Message validator. Stores the reference to the function to check the
   * message validity before actually performing the publishing. No invalid
   * messages can be published. Every message is valid when this field is empty.
   */
  ...
  /** Dynamic channel observer list. Represents the channel's observers list, it
   * can be empty or have listeners and subscribers mixed in any sequence. It
   * can be changed in runtime.
   */
  sys_slist_t *runtime_observers;
#endif /* CONFIG_ZBUS_RUNTIME_OBSERVERS_POOL_SIZE  */

  /** Channel observer list. Represents the channel's observers list, it can be
   * empty or have listeners and subscribers mixed in any sequence.
   */
 ...
 *
 * Every observer has an representation structure containing the relevant
 * information. An observer is a code portion interested in some channel. The
 * observer can be notified synchronously or asynchronously and it is called
 * listener and subscriber respectively. The observer can be enabled or disabled
 * during runtime by change the enabled boolean field of the structure. The
 * listeners have a callback function that is executed by the bus with the index
 * of the changed channel as argument when the notification is sent. The
 * subscribers have a message queue where the bus enqueues the index of the
 * changed channel when a notification is sent.
 *
 * @see zbus_obs_set_enable function to properly change the observer's enabled
 * field.
 *
 */
...
#define ZBUS_RUNTIME_OBSERVERS_LIST_DECL(_slist_name)                          \
  static sys_slist_t _slist_name
#define ZBUS_RUNTIME_OBSERVERS_LIST_INIT(_slist_name)                          \
  .runtime_observers = &_slist_name,
#else
#define ZBUS_RUNTIME_OBSERVERS_LIST_DECL(_slist_name)
 ...
 * This macro list the observers to be used in a file. Internally, it declares
 * the observers with the extern statement. Note it is only necessary when the
 * observers are declared outside the file.
 */
#define ZBUS_OBS_DECLARE(...) FOR_EACH(_ZBUS_OBS_EXTERN, (;), __VA_ARGS__)

/**
 * @def ZBUS_CHAN_DECLARE
 * This macro list the channels to be used in a file. Internally, it declares
 * the channels with the extern statement. Note it is only necessary when the
 * channels are declared outside the file.
 */
...
/**
 * @def ZBUS_OBSERVERS
 * This macro indicates the channel has listed observers. Note the sequence of
 * observer notification will follow the same as listed.
 */
 ...
 * @param _observers The observers list. The sequence indicates the priority of
...
#define ZBUS_CHAN_DEFINE(_name, _type, _validator, _user_data, _observers,     \
  ...
  static const struct zbus_observer *const _CONCAT(_zbus_observers_,           \
                                                   _name)[] = {                \
  ...
  const _ZBUS_STRUCT_DECLARE(zbus_channel, _name) = {                          \
      ...
      .message =                                                               \
          &_CONCAT(_zbus_message_, _name), /* Reference to the message */      \
      .validator = (_validator),           /* Validator function */            \
      .mutex = NULL,                       /* Channel's Mutex */               \
      ZBUS_RUNTIME_OBSERVERS_LIST_INIT(                                        \
          _CONCAT(_runtime_observers_, _name)) /* Runtime observer list */     \
          .observers =                                                         \
          _CONCAT(_zbus_observers_, _name)} /* Static observer list */

/**
 * @brief Initialize a message.
 *
 * This macro initializes a message by passing the values to initialize the
 * message struct or union.
 *
 * @param[in] _val Variadic with the initial values. ``ZBUS_INIT(0)`` means
 * ``{0}``, as ZBUS_INIT(.a=10, .b=30) means ``{.a=10, .b=30}``.
 */
 ...
 *
 * This macro defines an observer of subscriber type. It defines a message queue
 * where the subscriber will receive the notification asynchronously, and
 * initialize the ``struct zbus_observer`` defining the subscriber.
 *
 ...
 *
 * This macro defines an observer of listener type. This macro establishes the
 * callback where the listener will be notified synchronously, and initialize
 * the ``struct zbus_observer`` defining the listener.
 *
 ...
 * @param msg Reference to the message where the publish function copies the
 * channel's message data from.
 * @param timeout Waiting period to publish the channel,
 *                or one of the special values K_NO_WAIT and K_FOREVER.
 *
 * @retval 0 Channel published.
 * @retval -ENOMSG The message is invalid based on the validator function or
 * some of the observers could not receive the notification.
 * @retval -EBUSY The channel is busy.
 * @retval -EAGAIN Waiting period timed out.
 * @retval -EFAULT A parameter is incorrect, the notification could not be sent
 * to one or more observer, or the function context is invalid (inside an ISR).
 * The function only returns this value when the CONFIG_ZBUS_ASSERT_MOCK is
 * enabled.
 */
 ...
 * @param[out] msg Reference to the message where the read function copies the
 * channel's message data to.
 ...
 * @retval -EBUSY The channel is busy.
 * @retval -EAGAIN Waiting period timed out.
 * @retval -EFAULT A parameter is incorrect, or the function context is invalid
 * (inside an ISR). The function only returns this value when the
 * CONFIG_ZBUS_ASSERT_MOCK is enabled.
 */
 ...
 *
 * This routine claims a channel. During the claiming period the channel is
 * blocked for publishing, reading, notifying or claiming again. Finishing is
 ...
 *
 * This routine finishes a channel claim. After calling this routine with
 * success, the channel will be able to be used by other thread.
 *
 ...
 *
 * This routine forces the event dispatcher to notify the channel's observers
 * even if the message has no changes. Note this function could be useful after
 * claiming/finishing actions.
 *
 * @param chan The channel's reference.
 ...
 *
 * @warning This function must only be used directly for acquired (locked by
 * mutex) channels. This can be done inside a listener for the receiving channel
 * or after claim a channel.
 *
 * @param chan The channel's reference.
 *
 ...
 *
 * This routine returns a constant reference of a channel message. This should
 * be used inside listeners to access the message directly. In this way zbus
 * prevents the listener of changing the notifying channel's message during the
 ...
 * @retval -EALREADY The observer is already present in the channel's runtime
 * observers list.
 ...
 *
 * This routine changes the observer state. A channel when disabled will not
 * receive notifications from the event dispatcher.
 *
 * @param[in] obs The observer's reference.
 * @param[in] enabled State to be. When false the observer stops to receive
 * notifications.
 *
 ...
 *
 * This routine makes the subscriber to wait a notification. The notification
 * comes as a channel reference.
 *
 ...
 * iterator_func which is called for each channel. If the iterator_func returns
 * false all the iteration stops.
 *
...
bool zbus_iterate_over_channels(
    bool (*iterator_func)(struct zbus_channel *chan));

/**
 *
 * @brief Iterate over observers.
 *
 * Enables the developer to iterate over the observers giving to this function
 * an iterator_func which is called for each observer. If the iterator_func
 * returns false all the iteration stops.
 *
 * @retval true Iterator executed for all channels.
 * @retval false Iterator could not be executed. Some iterate returned false.
 */
bool zbus_iterate_over_observers(
    bool (*iterator_func)(struct zbus_observer *obs));
